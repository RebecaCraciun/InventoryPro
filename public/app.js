...
const showToast = (message, type = "info") => {
  ...
  if (type === "error") toastIcon.className += "fa-circle-xmark text-red-400";
    ...
    toastIcon.className += "fa-circle-check text-cyan-400";
...
searchBtn.addEventListener("click", async () => {
  ...
  if (!query)
    return showToast("Introdu un nume de produs pentru a căuta!", "error");

  const originalBtnHTML = searchBtn.innerHTML;
  searchBtn.innerHTML =
    '<i class="fa-solid fa-circle-notch fa-spin"></i> Se caută...';
  ...
  try {
    ...
    const BASE =
      window.location.hostname === "localhost"
        ? ""
        : "https://inventorypro-1-2n09.onrender.com";
...
saveBtn.addEventListener("click", async () => {
  ...
  if (!price || !quantity)
    return showToast("Te rog completează prețul și cantitatea!", "error");
